  ...
  if(this.clubName == this.pendValue){
    ...
    this.presentLoadingClub().then(() => {
      ...
      goToDetails(record: Record,i:number) {
        this.tab11=true;
        this.sports=true;
      this.router.navigate(['event-details'], { state: {sports:this.sports, record, bail: this.bails[i],leader:this.leader,tab1:this.tab1,tab11:this.tab11 } });
      }
      async openModal(){
