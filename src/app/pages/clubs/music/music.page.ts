  ...
  if(this.clubName == this.pendValue){
    ...
    this.presentLoadingClub().then(() => {
      ...
      goToDetails(record: Record,i:number) {
        this.tab11=true;
        this.music=true;
      this.router.navigate(['event-details'], { state: { music:this.music,record, bail: this.bails[i],leader:this.leader,tab1:this.tab1,tab11:this.tab11 } });
      }
      async openModal(){
